 ...
 * @link https://github.com/Descolada/AHK-v2-libraries/blob/main/Lib/String.ahk
 * @created 27.08.22
 * @author Descolada
 * @author tidbit /--- Author of "String Things - Common String & Array Functions", from which I copied/based a lot of methods
 * @author AfterLemon, Bon, Lexikos, MasterFocus, Rseding91, Verdlin
 * @abstract A compilation of useful string methods. Also lets strings be treated as objects.
 * @note These methods cannot be used as stand-alone. To do that, you must add another argument 'string' to the function and replace all occurrences of 'this' with 'string'.
 * @example
	==============================================================================
	| Methods                                                                    |
	|============================================================================|
...
class String2 {
	static __New() {
		; Add all String2 methods to String prototype
		; Note: Must use Object.Prototype.DefineProp for primitive prototypes
		for methodName in this.OwnProps() {
			; Skip __New and special properties handled separately
			if methodName ~= "^(__New|__Item|Length|WLength)$" {
				continue
			}
			
			if HasMethod(this, methodName) {
				Object.Prototype.DefineProp.Call(String.Prototype, methodName, {
					Call: this.%methodName%
				})
			}
		}
		
		; Add special property accessors
		Object.Prototype.DefineProp.Call(String.Prototype, "__Item", {get: (args*) => String2.__Item[args*]})
		Object.Prototype.DefineProp.Call(String.Prototype, "Length", {get: (arg) => String2.Length(arg)})
		Object.Prototype.DefineProp.Call(String.Prototype, "WLength", {get: (arg) => String2.WLength(arg)})
	}
